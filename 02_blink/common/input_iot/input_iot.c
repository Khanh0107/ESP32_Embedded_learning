 ...
 * Triggered on BOTH edges (press and release)
 */
static void IRAM_ATTR gpio_input_handler(void* arg) {
    int gpio_num = (uint32_t) arg;

    // Get current RTOS tick (in ISR-safe way)
    uint32_t rtc = xTaskGetTickCountFromISR();  // ms

    if (gpio_get_level(gpio_num) == 0)  // bat dau bam
    {
        // Start timeout timer (e.g., 5s)
        _start = rtc;
        xTimerStartFromISR(xTimers, 0);
    }
    else                                 // tha tay ra
    {
        // Stop timeout timer because user released before timeout
        xTimerStopFromISR(xTimers, 0);
        _stop = rtc;
        // Calculate press duration in tick
...
void input_set_timeout_button(void * cb)
{
    // Store the user callback function pointer for timeout button events
